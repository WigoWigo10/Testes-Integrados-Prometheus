...
if not exist "%RELATORIO_PATH%\PowerHub\HDMI" (
    mkdir "%RELATORIO_PATH%\PowerHub\HDMI"
    echo Pasta AC criada em "%RELATORIO_PATH%\PowerHub\HDMI".
) else (
...
set RELATORIO_TXT=%RELATORIO_PATH%\PowerHub\HDMI\Relatorio de Testes.txt
set RELATORIO_CSV=%RELATORIO_PATH%\PowerHub\HDMI\Relatorio de Testes.csv
set num=0

:checkfile
if exist "%RELATORIO_TXT%" (
    set /a num+=1
    set RELATORIO_TXT=%RELATORIO_PATH%\PowerHub\HDMI\Relatorio de Testes - !num!.txt
    set RELATORIO_CSV=%RELATORIO_PATH%\PowerHub\HDMI\Relatorio de Testes - !num!.csv
    goto checkfile
)
...
echo Criando done.txt em "%RELATORIO_PATH%\PowerHub\HDMI".
echo Teste concluído > "%RELATORIO_PATH%\PowerHub\HDMI\done.txt"

if exist "%RELATORIO_PATH%\PowerHub\HDMI\done.txt" (
