...
class HDMITest:
    ...
    def executar_hdmi(self):
        comando = f'HDMI_TestedeAtivacao.bat "{self.relatorio_path}" "{self.segundos}"'
        try:
            print(f"Executando comando: {comando}")  # Debug
            resultado = subprocess.run(comando, shell=True, check=True, capture_output=True, text=True)
            print(f"Saída: {resultado.stdout}")
        except subprocess.CalledProcessError as e:
            self.exibir_mensagem("erro", f"Erro ao executar o teste HDMI: {e}\nSaída de erro: {e.stderr}")
    ...
    def exibir_mensagem(self, tipo, mensagem):
        ...
        if tipo == "conclusão":
            ...
            msg_box.setInformativeText(f"O teste de HDMI foi concluído.")
        elif tipo == "erro":
